...
public class FlagUIVars : UIVars {
	...
	//base constructor
	public FlagUIVars(string resourceName, string n, string f, string i, bool showC = false, List<string> afts = null) : base("flag", resourceName, n, f, i) {
		if (afts == null) {
			afts = new List<string>() {"minefield"};
		}
		showCount = showC;
		allowedFloorTypes = afts;
	}
	//for placeable
    public FlagUIVars(string resourceName, string n, string f, string i, Type type, bool showC = true, List<string> afts = null) : this(resourceName, n, f, i, showC, afts) {
        spriteType = type;
    }
	//for consumable
    public FlagUIVars(string resourceName, string n, string f, string i, int cdc, bool showC = true, List<string> afts = null) : this(resourceName, n, f, i, showC, afts) {
...
public class UIManager : MonoBehaviour
{
    ...
    private void Awake() {
        ...
        flagUIVars = new Dictionary<Type, FlagUIVars>{
            //placeable flags
            {typeof(Base), new FlagUIVars("base", "Flag", "Nostalgic, isn't it?", "Drag and drop these wherever you think mines are.", typeof(BaseSprite), true, new List<string>{"minefield", "trial"})},
            ...
            {typeof(You), new FlagUIVars("you", "You Flag", "For use in the worst case scenario", "Revive yourself whenever you die", typeof(YouSprite), true, new List<string>{"minefield", "trial"})},
            ...
            //consumable flags
            {typeof(Chocolate), new FlagUIVars("chocolate", "Chocolate Flag", "Curiosity made the cat stronger.", "ONE TIME USE - Sacrifice half your health rounded up for two random flags.", 1)},
            {typeof(Dog), new FlagUIVars("dog", "Dog Flag", "Oh, how the tables have turned.", "For your next movement, you are immune to mines, but take damage from safe tiles.", 10)},
            {typeof(Shovel), new FlagUIVars("shovel", "Shovel Flag", "An upgrade for your tiny claws.", "ONE TIME USE - Immeditaely skip to the start of the next floor.", 1)},
        };
